...
std::optional<const DigitalStage::device_t> DigitalStage::Store::getLocalDevice()
{
  std::lock_guard<std::recursive_mutex>(this->local_device_mutex_);
...
void DigitalStage::Store::createCustomGroupPosition(const nlohmann::json customGroup)
{
...
DigitalStage::Store::readCustomGroupPosition(const std::string& id)
{
  ...
  if(this->customGroupPositions_.count(id) > 0)
    return this->customGroupPositions_[id].get<DigitalStage::custom_group_position_t>();
  return std::nullopt;
}
...
void DigitalStage::Store::createCustomGroupVolume(const nlohmann::json customGroup)
{
...
DigitalStage::Store::readCustomGroupVolume(const std::string& id)
{
  ...
  if(this->customGroupVolumes_.count(id) > 0)
    return this->customGroupVolumes_[id].get<DigitalStage::custom_group_volume_t>();
  return std::nullopt;
}
...
void DigitalStage::Store::updateCustomStageMemberVolume(const std::string& id,
                                                   const nlohmann::json update)
{
  std::lock_guard<std::recursive_mutex>(
      this->custom_stage_member_volumes_mutex_);
...
void DigitalStage::Store::createRemoteOvTrack(const nlohmann::json remoteOvTrack)
{
...
void DigitalStage::Store::removeCustomRemoteOvTrackPosition(const std::string& id)
{
  auto lock = std::unique_lock<std::recursive_mutex>(
      this->custom_remote_ov_track_positions_mutex_);
...
std::optional<const DigitalStage::stage_t> DigitalStage::Store::getCurrentStage()
{
  std::lock_guard<std::recursive_mutex>(this->stages_mutex_);
    ...
    const std::string& stageMemberId)
{
  ...
  if(this->customStageMemberVolumeIdByStageMemberId_.count(stageMemberId) > 0) {
    const std::string customStageMemberId =
        this->customStageMemberVolumeIdByStageMemberId_[stageMemberId];
    std::optional<const DigitalStage::custom_stage_member_volume_t> customStageMember =
...
std::optional<const DigitalStage::custom_remote_ov_track_volume_t>
DigitalStage::Store::getCustomOvTrackVolumeByOvTrackId(const std::string& ovTrackId)
{
  auto lock = std::unique_lock<std::recursive_mutex>(
      this->custom_remote_ov_track_volumes_mutex_);
