...
class AudioMixer {
 public:
  explicit AudioMixer(const std::weak_ptr<DigitalStage::Api::Client>& client_ptr, bool use_balance = false);
 ...
 private:
  void attachHandlers(const std::weak_ptr<DigitalStage::Api::Client>& client_ptr);
